        ...
        if serial_queue.qsize() > 0:
            ...
            if((index - self.counter) == 1 or (self.counter - index) == ((256*self.queue_prescaler) - 1)):
                ...
                try:
                    ...
                    band_10Hz = frequency_analysis[(frequency_analysis['frequency'] > 9.9) & (frequency_analysis['frequency'] < 10.1)].copy()
                    band_15Hz = frequency_analysis[(frequency_analysis['frequency'] > 14.9) & (frequency_analysis['frequency'] < 15.1)].copy()
                    band_20Hz = frequency_analysis[(frequency_analysis['frequency'] > 19.9) & (frequency_analysis['frequency'] < 20.1)].copy()
                    band_25Hz = frequency_analysis[(frequency_analysis['frequency'] > 24.9) & (frequency_analysis['frequency'] < 25.1)].copy()
                    amplitude_list = []
                    ...
                    if SSVEP_controller.max()['amplitude'] > 0.80:
                        target_band = SSVEP_controller[SSVEP_controller['amplitude'] > 0.80]['band'].iloc[0]

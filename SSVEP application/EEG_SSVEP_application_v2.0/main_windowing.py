...
class processing_socket(Frame):
    ...
    def listen(self):
        ...
        signal_plotter.print("[INFO] Socket connected: {}".format(addr))
    
    def send(self, direction):
        self.conn.send(str(direction))
...
def updatePreference():
    ...
    restart_serial(buffer = serial_reader.buffer)
...
class signalViewer(Frame):
    global serial_queue, preference_info, snake_game
    def __init__(self, master):
        Frame.__init__(self, master)
        ...
        self.packet_size = int(self.real_packet_size/self.queue_prescaler)
    ...
    def first_run(self):
        ...
        while True:
            if serial_queue.qsize() > 0:
                singlePacket = packet(ast.literal_eval(serial_queue.get(0)))
    ...
    def run_async_task(self):
        ...
        if serial_queue.qsize() > 0:
            ...
            if((index - self.counter) == 1 or (self.counter - index) == ((256*self.queue_prescaler) - 1)):
                ...
                fft_x = xf[:int(self.frequency_limit*float(T)*self.packet_size*self.packet_number_frequency)]
                fft_y = (2.0/(float(self.packet_size)*self.packet_number_frequency) * np.abs(yf[:int(float(self.packet_size)*self.packet_number_frequency)//2]))[:int(self.frequency_limit*float(T)*self.packet_size*self.packet_number_frequency)]
                try:
                    frequency_analysis = pd.DataFrame(data = {'frequency': fft_x, 'amplitude': fft_y})
                    band_10Hz = frequency_analysis[(frequency_analysis['frequency'] > 9.0) & (frequency_analysis['frequency'] < 11.0)].copy()
                    band_15Hz = frequency_analysis[(frequency_analysis['frequency'] > 14.0) & (frequency_analysis['frequency'] < 16.0)].copy()
                    band_20Hz = frequency_analysis[(frequency_analysis['frequency'] > 19.0) & (frequency_analysis['frequency'] < 21.0)].copy()
                    band_25Hz = frequency_analysis[(frequency_analysis['frequency'] > 24.0) & (frequency_analysis['frequency'] < 26.0)].copy()
                    amplitude_list = []
                    amplitude_list.append(band_10Hz.max()['amplitude'])
                    amplitude_list.append(band_15Hz.max()['amplitude'])
                    amplitude_list.append(band_20Hz.max()['amplitude'])
                    amplitude_list.append(band_25Hz.max()['amplitude'])
                    SSVEP_controller = pd.DataFrame(data = {'band': [10, 15, 20, 25], 'amplitude': amplitude_list})
                    if (SSVEP_controller.max()['amplitude'] - SSVEP_controller.min()['amplitude']) > 0.5:
                        target_band = SSVEP_controller[SSVEP_controller['amplitude'] > 1.0]['band'].iloc[0]
                        print("SSVEP trigger: {} Hz band".format(target_band))
                        if int(target_band) == 10:
                            snake_game.send(8)
                        elif int(target_band) == 15:
                            snake_game.send(4)
                        elif int(target_band) == 20:
                            snake_game.send(6)
                        elif int(target_band) == 25:
                            snake_game.send(2)
                except:
                    pass

                self.line2.set_xdata(fft_x)
                ...
                print("WARNING: packet {} lost".format(index))
        ...
        loop_time = getTimeMillis() - loop_start_time
        self.run_job = root.after(int(self.packet_time - loop_time), self.run_async_task)
            ...
            for line in reader:
                ...
                if self.verbose == 1:
                    ...
                    for i in prescaler_time_array:
                        ...
                        serial_queue.put_nowait(str(timestamp_packet))
                        prescaler_index+=1
...
def restart_serial(buffer = False):
    ...
    if ser.isOpen() or buffer:
        try:
            ...
            pass
        acquireFrame = Frame(root, width = 600, height = 500, background = "gray25")
        acquireFrame.pack_propagate(0)
        acquireFrame.pack(side = LEFT, padx = 10, pady = (40, 10))
        try:
            # ignore this error
            root.after_cancel(signal_viewer.run_job)
            signal_viewer.pack_forget()
            signal_viewer.destroy()
        except:
            pass
        ...
        signal_viewer = signalViewer(acquireFrame)
        signal_viewer.pack()
...
def start_processing():
    global snake_game
    signal_plotter.print("[INFO] Waiting for connection from SSVEP processing snake GUI")
    snake_game = processing_socket(root)
    snake_game.pack()
    thread.start_new_thread(snake_game.listen, ())
...
plotterFrame.pack(side = LEFT, padx = 10, pady = (0, 10))

ser = serial.Serial()
ser.port = 'COM1'
ser.baudrate = 115200

serial_reader = EEGReader(root, out_file = False, buffer = False)
serial_reader.pack()

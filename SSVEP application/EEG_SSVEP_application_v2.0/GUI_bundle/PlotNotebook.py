...
class plotter:
    def __init__(self, id):
        self.ID = int(id)
        self.plotArray = []
        self.figure = plt.figure(num = self.ID, figsize=(15,8), facecolor = "#404040")
        self.scatterSize = 1
        self.xlimA = 0
        self.xlimB = 0
        self.method = 'scatter'
        ...
        self.hold = False
        self.window_size = 10000 # 10000 : 10 seconds
    ...
    def plotLine(self):
        ...
        with plt.rc_context({'axes.edgecolor':'#FFFFFF', 'xtick.color':'#FFFFFF', 'ytick.color':'#FFFFFF'}):
            for i in range(0, len(self.plotArray)):
                plt.subplot(subplotNum, 1, self.plotArray[i].getIndex())
                if self.plotArray[i].getType() == "frequency_cursor":
                    plt.plot(self.plotArray[i].getX(), self.plotArray[i].getY(), label = self.plotArray[i].getLegend(), color='black', ls = '-.', linewidth = 2.0)
                else:
                ...
                if self.plotArray[i].getType() == 'frequency':
                    plt.xticks(np.linspace(self.xlimA, self.xlimB, 10), np.around(np.linspace(0, 50, 10), decimals = 2))
                    plt.ylim(0, 2)
                else:
                    axisTime = []
    ...
    def setFilter(self, xlimA, xlimB):
        ...
        for i in range(0, len(self.plotArray)):
            indexList.append(self.plotArray[i].getIndex())
            if(self.plotArray[i].getType() == "frequency"):
                plot_with_frequency.append(self.plotArray[i].getIndex())
                frequency_max_A = min(self.plotArray[i].getX())
                frequency_max_B = max(self.plotArray[i].getX())       

        subplotNum = len(set(indexList))

        for i in range(1, subplotNum + 1):
            try:
                plot_with_frequency.index(i)
                plt.subplot(subplotNum, 1, i)
                plt.xlim(frequency_max_A, frequency_max_B)
                plt.xticks(np.linspace(frequency_max_A, frequency_max_B, 10), np.around(np.linspace(0, 50, 10), decimals = 2))
            except ValueError:
                plt.subplot(subplotNum, 1, i)
                plt.xlim(self.xlimA, self.xlimB)
                plt.xticks(np.linspace(self.xlimA, self.xlimB, 4), axisTime)
    ...
    def eventHandler(self, event):
        ...
        mouseY = 450 - int(eventArray[3])
        plotCount = self.getPlotCount()
        thresholdY = 450.0/plotCount
        eventOnPlot = int(float(mouseY)/thresholdY) + 1
        ### New function: Only receive event for plots that are not type "frequency"
        plot_with_frequency = []
        for i in range(0, len(self.plotArray)):
            if(self.plotArray[i].getType() == "frequency"):
                plot_with_frequency.append(self.plotArray[i].getIndex())
        try:
            plot_with_frequency.index(eventOnPlot)
        except ValueError:
            ...
            if int(eventArray[1]) == 1:
                ...
                if self.filterHandler['counter'] == 0:
                    if self.window_size is not None:
                        ...
                        self.filterHandler['B'] = int(float(eventArray[4])) + self.window_size
                        self.drawCursor(event, type = 'filter')
                        self.filterHandler['counter']+=1
                    else:
                        self.filterHandler['A'] = int(float(eventArray[4]))
        ...
        for i in range(1, subplotNum + 1):
            try:
                ...
                if type == 'cursor':
                    ...
                    if self.filterHandler['counter'] == 0:
                        self.filterHandler['cursorA'].append(plt.plot([self.filterHandler['A'], self.filterHandler['A']], plt.gca().get_ylim(), color='yellow', ls = '-.'))
                    elif self.filterHandler['counter'] == 1:
                        self.filterHandler['cursorB'].append(plt.plot([self.filterHandler['B'], self.filterHandler['B']], plt.gca().get_ylim(), color='yellow', ls = '-.'))
    ...
    def zoom(self, event):
        plt.figure(self.ID)
        if not self.hold:
            zoom_constant = 0.1*(self.xlimB - self.xlimA)
            if self.xlimB - self.xlimA > 2*event.step*zoom_constant:
                self.xlimA += (event.step * zoom_constant)
                self.xlimB -= (event.step * zoom_constant)
                self.setFilter(self.xlimA, self.xlimB)
        else:
            ymin, ymax = plt.ylim()
            step_min = (event.ydata - ymin)*0.05
            step_max = (event.ydata - ymax)*0.05
            ymin += step_min*event.step
            ymax += step_max*event.step
            plt.ylim(ymin, ymax)
    
    def windowing(self, winA, winB):
        plt.figure(self.ID)
        indexList = []
        for i in range(0, len(self.plotArray)):
            indexList.append(self.plotArray[i].getIndex())
        ...
        try:
            for i in range(1, subplotNum + 1):
                try:
                    ...
                    plt.subplot(subplotNum, 1, i)
                    if self.filterHandler['counter'] == 2:
                        self.filterHandler['cursorA'].pop(0).pop(0).remove()
                        self.filterHandler['cursorB'].pop(0).pop(0).remove()
            self.xCursor.pop(0).remove()
        except Exception:
        ...
        for i in range(1, subplotNum + 1):
            try:
                ...
                plt.subplot(subplotNum, 1, i)
                plt.autoscale(False)
                if self.filterHandler['counter'] == 2: # check if there is window applied
                    self.filterHandler['A'] = winA
                    self.filterHandler['B'] = winB
                    self.filterHandler['cursorA'].append(plt.plot([self.filterHandler['A'], self.filterHandler['A']], plt.gca().get_ylim(), color='yellow', ls = '-.'))
                    self.filterHandler['cursorB'].append(plt.plot([self.filterHandler['B'], self.filterHandler['B']], plt.gca().get_ylim(), color='yellow', ls = '-.'))
...
class PlotNotebook(ttk.Notebook):
    ...
    def __initialize_custom_style(self):
        ...
        style.layout("CustomNotebook.TNotebook", [("CustomNotebook.TNotebook.client", {"sticky": "nswe"})])
        style.layout("CustomNotebook.TNotebook.Tab", [
                    ...
                    ("CustomNotebook.TNotebook.padding", {
                        ...
                        "children": [
                            ("CustomNotebook.TNotebook.focus", {
                                ...
                                "sticky": "nswe",
                                "children": [
                                    ("CustomNotebook.TNotebook.label", {"side": "left", "sticky": ''}),
                                    ("CustomNotebook.TNotebook.close", {"side": "left", "sticky": ''}),
                                ]
                            })
                        ]
                    })

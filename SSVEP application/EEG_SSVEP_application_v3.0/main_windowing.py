...
hand_state = False
previous_alpha_mean = None
streamer = None

class Streamer:
    def __init__(self, server_address = "192.168.0.107", port = "5555"):
        context = zmq.Context()
        self.footage_socket = context.socket(zmq.PUB)
        self.footage_socket.connect('tcp://' + server_address + ':' + port)

    def send(self, data):
        self.footage_socket.send_string(data)
...
class signalViewer(Frame):
    global serial_queue, preference_info, hand_state, previous_alpha_mean, streamer
    ...
    def run_async_task(self):
        global hand_state, previous_alpha_mean, streamer
        ...
        if serial_queue.qsize() > 0:
            ...
            if((index - self.counter) == 1 or (self.counter - index) == ((256*self.queue_prescaler) - 1)):
                ...
                try:
                    ...
                    alpha = frequency_analysis[(frequency_analysis['frequency'] > 8.0) & (frequency_analysis['frequency'] < 12.0)].copy()
                    alpha_mean = alpha["amplitude"].mean()
                    if alpha_mean > 1.0 and previous_alpha_mean <= 1.0:
                        if hand_state:
                            hand_state = False
                            # close hand
                            print("close hand")
                            ...
                            print("open hand")
                            streamer.send("2")
                    previous_alpha_mean = alpha_mean
                except:
                    pass
